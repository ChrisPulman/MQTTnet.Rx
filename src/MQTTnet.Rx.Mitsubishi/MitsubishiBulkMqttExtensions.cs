...
/// <summary>Provides MQTT bridges for Mitsubishi logical-tag bulk operations.</summary>
public static class MitsubishiBulkMqttExtensions
{
    /// <summary>Provides Mitsubishi logical-tag bulk bridges for MQTT client sequences.</summary>
    /// <param name="client">The MQTT client sequence.</param>
    extension(IObservable<IMqttClient> client)
    {
        /// <summary>Publishes observed Mitsubishi logical-tag values for many tags.</summary>
        /// <param name="topic">The MQTT topic that receives logical tag payloads.</param>
        /// <param name="logicalTags">The Mitsubishi logical-tag client.</param>
        /// <param name="tagNames">The logical tag names to observe.</param>
        /// <returns>The MQTT publish results.</returns>
        public IObservable<MqttClientPublishResult> PublishMitsubishiTags(
            string topic,
            MitsubishiLogicalTagClient logicalTags,
            params string[] tagNames) =>
            client.PublishMitsubishiTags(topic, logicalTags, FormatLogicalTagValue, tagNames);
        ...
        /// <summary>Writes MQTT payloads to multiple Mitsubishi logical tags.</summary>
        ...
        /// <param name="onError">A callback for source, conversion, or logical-tag write failures.</param>
        ...
        public IDisposable SubscribeMitsubishiTags(
            string topic,
            MitsubishiLogicalTagClient logicalTags,
            ...
            CancellationToken cancellationToken)
        {
            ...
            var observer = new MitsubishiBulkWriteObserver(logicalTags, payloadParser, onError, cancellationToken);
            observer.Attach(client.SubscribeToTopic(topic).Subscribe(observer));
            return observer;
        }
    }

    /// <summary>Provides Mitsubishi logical-tag bulk bridges for resilient MQTT client sequences.</summary>
    ...
    extension(IObservable<IResilientMqttClient> client)
    {
        ...
        public IObservable<ApplicationMessageProcessedEventArgs> PublishMitsubishiTags(
            string topic,
            MitsubishiLogicalTagClient logicalTags,
            params string[] tagNames)
        {
    ...
    /// <summary>Provides Mitsubishi logical-tag bulk bridges for asynchronous MQTT client sequences.</summary>
    ...
    extension(IObservableAsync<IMqttClient> client)
    {
        ...
        /// <param name="tagNames">The logical tag names to observe.</param>
        /// <returns>The MQTT publish results.</returns>
        public IObservableAsync<MqttClientPublishResult> PublishMitsubishiTags(
            string topic,
            MitsubishiLogicalTagClient logicalTags,
            params string[] tagNames) =>
            ObservableSignalConversion.ToSignal(client.ToObservable().PublishMitsubishiTags(topic, logicalTags, tagNames));
        ...
        public IObservableAsync<MqttClientPublishResult> PublishMitsubishiTags(
            ...
            ObservableSignalConversion.ToSignal(
                client.ToObservable().PublishMitsubishiTags(topic, logicalTags, payloadFormatter, tagNames));
        ...
        public IDisposable SubscribeMitsubishiTags(
            ...
            Action<Exception>? onError,
            CancellationToken cancellationToken) =>
            client.ToObservable().SubscribeMitsubishiTags(topic, logicalTags, payloadParser, onError, cancellationToken);
    }

    /// <summary>Provides Mitsubishi logical-tag bulk bridges for asynchronous resilient MQTT client sequences.</summary>
    ...
    extension(IObservableAsync<IResilientMqttClient> client)
    {
        ...
        /// <param name="tagNames">The logical tag names to observe.</param>
        /// <returns>The resilient MQTT publish results.</returns>
        public IObservableAsync<ApplicationMessageProcessedEventArgs> PublishMitsubishiTags(
            string topic,
            MitsubishiLogicalTagClient logicalTags,
            params string[] tagNames) =>
            ObservableSignalConversion.ToSignal(client.ToObservable().PublishMitsubishiTags(topic, logicalTags, tagNames));
    ...
    private sealed class MitsubishiBulkWriteObserver(
        MitsubishiLogicalTagClient logicalTags,
        Func<string, IReadOnlyCollection<LogicalTagValue>> payloadParser,
        Action<Exception>? onError,
        CancellationToken cancellationToken)
        : IObserver<MqttApplicationMessageReceivedEventArgs>, IDisposable
    {
        ...
        private async Task WriteAfterAsync(Task previous, string payload, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var results = await logicalTags.WriteManyAsync(values, cancellationToken).ConfigureAwait(false);
